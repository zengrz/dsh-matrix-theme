 ...
 * contributes its three surfaces — the General-section toggle row, the rain
 * quality row, and the frame-wide digital-rain backdrop — all reading one
 * shared mirror store of the theme snapshot. The mirror fields' only writer
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface LocaleNamespaceMap {
    /** The Matrix toggle and rain quality rows' copy. */
    matrix: MatrixKey
  }
}
...
export function apply(ctx: ClientContext): void {
  ...
  // One shared store behind every entry: whichever entry's bound actions
  ...
  const qualityInject = (actions: BoundActions<typeof store>): MatrixQualityRowInjected => {
    bound = actions
    sync(ctx.theme.getTheme())
    return {
      setQuality: (quality) => {
