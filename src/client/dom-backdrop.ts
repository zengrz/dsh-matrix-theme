 ...
 * veil) and reuses the same engine, the same `prefers-reduced-motion` gate, the
 * same theme-active condition, and the same rain quality level. The veil
 * opacity and the rain quality are read from localStorage and updated on
 * `storage` / custom events so the settings rows stay live on this path too.
 * The disposer removes the layer, the promoted-<style> tag, and the engine;
 * the caller (the plugin's apply closure) runs it from a `setTimeout(0)` that
 * fires after synchronous boot so ui-layout has already declared its slots
 * (or not) by the time the check runs.
 */
import type { ClientContext } from '@deepseek-ai/dsh-client-runtime/client'
import { THEME_ID } from './matrix-tokens.ts'
import { loadRainQuality, type RainQuality } from './rain-quality.ts'
...
export function mountDomBackdrop(ctx: ClientContext): () => void {
  ...
  let quality: RainQuality = loadRainQuality()
  ...
  // Live-update the engine when the quality row writes (same tab via custom
  // event, cross-tab via storage event).
...
export function notifyQualityChange(): void {
