	...
	factory: (require) => {
		...
		*/
			...
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					...
					setOpacity: (d, opacity) => {
						...
						if (typeof localStorage !== "undefined") localStorage.setItem(OPACITY_STORAGE_KEY, String(d.opacity));
					}
				}
			});
		}
		...
		var MatrixRow_module_css_default = {
			...
			"group": "Gk7Z5q_group",
			"title": "Gk7Z5q_title",
			"toggle": "Gk7Z5q_toggle", "opacityRow": "Gk7Z5q_opacityRow", "opacityLabel": "Gk7Z5q_opacityLabel", "slider": "Gk7Z5q_slider"
		};
		//#endregion
		...
		*/
		function apply(ctx) {
			...
			*/
			const OPACITY_EVT = "dsh-matrix-theme:opacity";
			function notifyOpacityChange() { window.dispatchEvent(new Event(OPACITY_EVT)); }
			function mountDomBackdrop(ctx) {
				...
				window.addEventListener("resize", onResize);
				const onOpacityEvt = () => { veil.style.backgroundColor = `rgba(0, 0, 0, ${readOpacity()})`; };
				window.addEventListener(OPACITY_EVT, onOpacityEvt);
				window.addEventListener("storage", onOpacityEvt);
				return () => {
					window.removeEventListener("resize", onResize);
					window.removeEventListener(OPACITY_EVT, onOpacityEvt);
					window.removeEventListener("storage", onOpacityEvt);
					offTheme();
					engine?.dispose();
					layer.remove();
					promoted.remove();
