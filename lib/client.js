window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
			...
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: {
					...
					setOpacity: (d, opacity) => {
						...
						if (typeof localStorage !== "undefined") localStorage.setItem(OPACITY_STORAGE_KEY$1, String(d.opacity));
					}
				}
			});
		}
		...
		const css$2 = ".Gk7Z5q_group{border-bottom:1px solid var(--dsw-alias-border-l2);flex-direction:column;gap:8px;padding:16px 0;display:flex}.Gk7Z5q_title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}.Gk7Z5q_toggle{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l2);font:inherit;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border-radius:12px;align-items:center;gap:8px;padding:4px;font-size:14px;line-height:22px;display:flex}.Gk7Z5q_knob{box-sizing:border-box;background:var(--dsw-alias-label-secondary);width:16px;height:16px;transition:background var(--ds-transition-duration-slow) var(--ds-ease-in-out);border-radius:8px}.Gk7Z5q_toggle.Gk7Z5q_on .Gk7Z5q_knob{background:var(--dsw-alias-state-success-primary)}.Gk7Z5q_toggle:hover{background:var(--dsw-alias-interactive-bg-hover)}.Gk7Z5q_opacityRow{align-items:center;gap:12px;display:flex}.Gk7Z5q_opacityLabel{color:var(--dsw-alias-label-tertiary);white-space:nowrap;font-size:13px;line-height:20px}.Gk7Z5q_slider{-webkit-appearance:none;appearance:none;background:var(--dsw-alias-border-l3);cursor:pointer;border-radius:2px;outline:none;flex:1;height:4px}.Gk7Z5q_slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;background:var(--dsw-alias-brand-primary);cursor:pointer;border:none;border-radius:50%;width:14px;height:14px}.Gk7Z5q_slider::-moz-range-thumb{background:var(--dsw-alias-brand-primary);cursor:pointer;border:none;border-radius:50%;width:14px;height:14px}";
		...
		var MatrixRow_module_css_default = {
			"slider": "Gk7Z5q_slider",
			"toggle": "Gk7Z5q_toggle",
			"opacityLabel": "Gk7Z5q_opacityLabel",
			"knob": "Gk7Z5q_knob",
			"on": "Gk7Z5q_on",
			"group": "Gk7Z5q_group",
			"opacityRow": "Gk7Z5q_opacityRow",
			"title": "Gk7Z5q_title"
		};
		//#endregion
		...
		*/
		function MatrixRow({ t, setMatrix, setOpacity, useStore }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					on && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: MatrixRow_module_css_default.opacityRow,
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: MatrixRow_module_css_default.opacityLabel,
							children: t("matrix.opacity")
							...
							step: .05,
							value: opacity,
							className: MatrixRow_module_css_default.slider,
							onChange: (e) => {
		...
		*/
		...
		/** Custom event dispatched on rain quality write (for same-tab updates). */
		const QUALITY_EVENT = "dsh-matrix-theme:quality";
		...
		function readOpacity() {
			...
			const v = Number.parseFloat(raw);
			return Number.isNaN(v) ? .5 : Math.max(0, Math.min(1, v));
		}
		/**
		...
		*/
		function mountDomBackdrop(ctx) {
			...
			const sync = () => {
				...
				if (active && quality !== "off" && engine === null) {
					engine = new RainEngine(canvas, quality);
					engine.start();
				} else if ((!active || quality === "off") && engine !== null) {
					engine.dispose();
					engine = null;
				}
			};
			sync();
			const onQuality = () => {
				quality = loadRainQuality();
				sync();
			};
			window.addEventListener(QUALITY_EVENT, onQuality);
			window.addEventListener("storage", onQuality);
			...
			return () => {
				...
				window.removeEventListener("storage", onQuality);
				offTheme();
				engine?.dispose();
				layer.remove();
				promoted.remove();
		...
		function notifyOpacityChange() {
			window.dispatchEvent(new Event(OPACITY_EVENT));
		}
		/** Dispatch the custom event so the DOM fallback rebuilds its engine on quality change. */
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				const timer = setTimeout(() => {
					if (disposed || ctx.slots.spec("shell.backdrop") !== void 0) return;
